package com.knms.shop.android.core.compress;
...
public class Engine {
    ...
    File compress() throws IOException {
        ...
        if(srcImg.length() < 1 * 1024 * 1024){
            tagBitmap.compress(Bitmap.CompressFormat.JPEG, 75, stream);
        }else if(srcImg.length() < 2 * 1024 * 1024){
            tagBitmap.compress(Bitmap.CompressFormat.JPEG, 55, stream);
        }else if(srcImg.length() < 4 * 1024 * 1024){
