...
public class WheelView extends View {
    ...
    BaseWheelAdapter adapter;

    private String label;//附加单位
    int labelRight;//label右边偏移
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 20, displayMetrics);//默认大小

        float density = displayMetrics.density; // 屏幕密度（0.75/1.0/1.5/2.0/3.0）
    ...
    private void remeasure() {//重新测量
        ...
        if (0 == itemsVisible % 2) ++this.itemsVisible;
    ...
    public final void setTextSize(float size) {
        if (size > 0.0F ) {
            textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, size, displayMetrics);
            paintOuterText.setTextSize(textSize);
            paintCenterText.setTextSize(textSize);
        }
    }
    ...
    public final void setAdapter(BaseWheelAdapter adapter) {
        if (null != this.adapter && null != observer){
...
//        if (null == adapter) return 0;
        if (null == oldCenterText) return initPosition;
        return adapter.indexOf(oldCenterText);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(label)&& isCenterLabel) {
            ...
            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET - labelRight, centerY, paintCenterText);
        }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    oldCenterText = visibles[counter];
    ...
    public void setLabelRight(int labelRight) {
        this.labelRight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, labelRight, displayMetrics);
    }
    ...
    /** 手势监听 */
     public static class LoopViewGestureListener extends android.view.GestureDetector.SimpleOnGestureListener {
    ...
    class InertiaTimerTask extends TimerTask {
        ...
        public final void run() {
            ...
            if (Math.abs(a) >= 0.0F && Math.abs(a) <= 20F) {
                ...
                loopView.handler.sendEmptyMessage(WheelView.MessageHandler.WHAT_SMOOTH_SCROLL);
                return;
            }
            ...
            loopView.handler.sendEmptyMessage(WheelView.MessageHandler.WHAT_INVALIDATE_LOOP_VIEW);
        }
    ...
    class SmoothScrollTimerTask extends TimerTask {
        ...
        public final void run() {
            ...
            if (Math.abs(realTotalOffset) <= 1) {
                loopView.cancelFuture();
                loopView.handler.sendEmptyMessage(WheelView.MessageHandler.WHAT_ITEM_SELECTED);
            } else {
    ...
    DataSetObserver observer = new DataSetObserver() {
        ...
        public void onChanged() {
            ...
            initPosition = adapter.indexOf(oldCenterText);
            if (initPosition >= adapter.getItemsCount()) initPosition = adapter.getItemsCount()-1;
            totalScrollY = 0;
            requestLayout();
            invalidate();
        }
    ...
    /** 适配器 */
    public static abstract class BaseWheelAdapter<T> {
