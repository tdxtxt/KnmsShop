...
public class MoonUtil {
	 ...
	 */
	public static void viewSetText(View textView, SpannableString mSpannableString) {
	...
	public static SpannableString makeSpannableStringTags(Context context, String value, float scale, int align, boolean bTagClickable) {
		...
		setHtmlHighlight(mSpannableString);
		setPhoneHighlight(mSpannableString);
	...
	public static void setHtmlHighlight(SpannableString spannableString){
		int start =0;
		int end = 0;
		String value = spannableString.toString();
		Pattern pattern = Pattern.compile("(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]");
		Matcher htmlMatcher = pattern.matcher(value);
		while (htmlMatcher.find()) {
			start = htmlMatcher.start();
			end = htmlMatcher.end();
			String url = htmlMatcher.group();
			spannableString.setSpan(getClickableSpanUrl(url), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
	}
	public static void setPhoneHighlight(SpannableString spannableString){
		setMoblieNumHighlight(spannableString);
		setTelephoneNumHighlight(spannableString);
	}
	private static void setMoblieNumHighlight(SpannableString spannableString){
		int start = 0;
		int end = 0;
		String value = spannableString.toString();
		Pattern patternPhone = Pattern.compile("[1][34578]\\d{9}");
		Matcher phoneMatcher = patternPhone.matcher(value);
		while (phoneMatcher.find()) {
			start = phoneMatcher.start();
			end = phoneMatcher.end();
			String url = phoneMatcher.group();
			spannableString.setSpan(getClickableSpanPhone(url), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
	}
	private static void setTelephoneNumHighlight(SpannableString spannableString){
		int start = 0;
		int end = 0;
		String value = spannableString.toString();
		Pattern patternPhone = Pattern.compile("(\\d{3}-)?\\d{8}");
		Matcher phoneMatcher = patternPhone.matcher(value);
		while (phoneMatcher.find()) {
			start = phoneMatcher.start();
			end = phoneMatcher.end();
	...
	private static ClickableSpan getClickableSpanUrl(final String url) {
	...
	private static ClickableSpan getClickableSpanPhone(final String phone) {
		return new ClickableSpan() {
			@Override
			public void onClick(View widget) {
				final Context context = widget.getContext();
				CommonHelper.onCallPhone((Activity) context,phone);
			}
			@Override
			public void updateDrawState(TextPaint ds) {
				ds.setColor(Color.BLUE); //设置链接的文本颜色
