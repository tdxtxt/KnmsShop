...
public class TimeLineView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 1; i < mPointTxt.size(); i++) {
            drawLine(canvas,i , this.mStep > i, this.mXpoints[(i - 1)], this.mXpoints[i]);
    ...
    private void drawCircle(Canvas canvas, int drawStep, String text, int dx) {
        ...
        canvas.drawCircle(dx,getHeight() - this.mRadius - 9,drawStep==this.mStep?this.mRadius+5:this.mRadius, this.mPaint);
        this.mPaint.setStyle(Paint.Style.FILL);
        this.mPaint.setColor(this.mStep == drawStep ? getResources().getColor(R.color.white_ffffff) : this.mStep > drawStep ? getResources().getColor(R.color.white_ffffff) : getResources().getColor(R.color.common_gray));
        canvas.drawCircle(dx, getHeight() - this.mRadius - 9, drawStep==this.mStep?this.mRadius-8:this.mRadius-12, this.mPaint);
        ...
        canvas.drawText(text, dx - getWordCount(text) * this.mTextSize, getHeight() - this.mRadius * 2 - 60, this.mPaint);
    }

    private void drawLine(Canvas paramCanvas, int drawStep,boolean isStart, int startX, int endX) {
        this.mPaint.setColor(isStart ? mStartedLineColor : mPreLineColor);
        this.mPaint.setStyle(Paint.Style.FILL);
        this.mPaint.setStrokeWidth(mLineWidth);
        paramCanvas.drawLine(drawStep==this.mStep?this.mRadius+ startX+5:this.mRadius+ startX, getHeight() - this.mRadius - 9, endX - this.mRadius, getHeight() - this.mRadius - 9, this.mPaint);
    }
