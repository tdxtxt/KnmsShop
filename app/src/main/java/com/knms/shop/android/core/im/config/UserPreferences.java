package com.knms.shop.android.core.im.config;
...
public class UserPreferences {
    ...
    private static StatusBarNotificationConfig getConfig(String key) {
        return SPUtils.getSerializable(key,StatusBarNotificationConfig.class);
    }

    private static void saveStatusBarNotificationConfig(String key , StatusBarNotificationConfig config) {
        SPUtils.saveSerializable(key,config);
    }
    ...
    static SharedPreferences getSharedPreferences() {
        SharedPreferences sp = KnmsShopApp.getInstance().getSharedPreferences("account", Context.MODE_PRIVATE);
        return sp;
    }
}
